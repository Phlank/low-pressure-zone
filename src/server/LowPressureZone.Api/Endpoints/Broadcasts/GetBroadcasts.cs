...
public class GetBroadcasts(AzuraCastClient client, DataContext context) : EndpointWithoutRequest<IEnumerable<BroadcastResponse>, BroadcastMapper>
{
    ...
    public override async Task HandleAsync(CancellationToken ct)
    {
        ...
        if (!broadcastResult.IsSuccess)
        {
            if (broadcastResult.Error is not null)
                ThrowError(broadcastResult.Error.ReasonPhrase ?? "Unknown reason", (int)broadcastResult.Error.StatusCode);

            await SendForbiddenAsync(ct);
        }

        var broadcasts = broadcastResult.Data;
        ...
        {
            var overlappingSlot = timeslots.FirstOrDefault(timeslot => timeslot.StartsAt.AddMinutes(-BroadcastBufferMinutes) < response.Start
                                                                       && timeslot.StartsAt.AddMinutes(BroadcastBufferMinutes) > response.Start);
