...
public static class RuleBuilderExtensions
{
    ...
    {
        return ruleBuilder.NotEmpty().WithMessage(Errors.Required).Custom((username, context) =>
        {
    ...
    {
        return ruleBuilder.MinimumLength(8).WithMessage(Errors.MinLength(8)).Custom(static (password, context) =>
        {
            if (!PasswordNumberRegex.IsMatch(password))
            {
                context.AddFailure(nameof(password), Errors.PasswordNumber);
            }
            ...
            {
                context.AddFailure(nameof(password), Errors.PasswordUppeercase);
            }
        });
    }
}
