...
{
    private static readonly Regex InvalidUsernameCharactersRegex = new("[^A-Za-z0-9-._@+]", RegexOptions.Compiled);

    private static readonly Regex PasswordNumberRegex = new("[0-9]", RegexOptions.Compiled);
    private static readonly Regex PasswordSpecialCharacterRegex = new("[^A-Za-z0-9]", RegexOptions.Compiled);
    ...
    public static IRuleBuilder<T, string> Password<T>(this IRuleBuilder<T, string> ruleBuilder)
        => ruleBuilder.MinimumLength(8).WithMessage(Errors.MinLength(8)).Custom(static (password, context) =>
        {
            ...
            if (!PasswordSpecialCharacterRegex.IsMatch(password))
                context.AddFailure(nameof(password), Errors.PasswordSymbol);
            if (!PasswordLowercaseRegex.IsMatch(password))
                context.AddFailure(nameof(password), Errors.PasswordLowercase);
            if (!PasswordUppercaseRegex.IsMatch(password))
                context.AddFailure(nameof(password), Errors.PasswordUppercase);
        });
}