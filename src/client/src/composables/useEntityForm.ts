 ...
 * @param updatePersistentFn Function to update an existing persistent entity in the store. See `useUpdatePersistentItemFn`.
 * @param onSubmitted Callback function to be called after successful submission.
 * @param useProgress Will provide a progress `Ref<number>` to the create/update functions if `true`. Only applies to xhr requests.
 *
...
>(
  ...
  const apiAction = async (entity?: TEntity) => {
    ...
    if (entity !== undefined && updatePersistentFn) {
      if (useProgress) result = await updatePersistentFn(entity.id, state, val, progress)
      else result = await updatePersistentFn(entity.id, state, val)
    } else if (entity === undefined && createPersistentFn) {
      if (useProgress) result = await createPersistentFn(state, val, progress)
      else result = await createPersistentFn(state, val)
    }
